...
var __require2 = ((x2) => typeof __require < "u" ? __require : typeof Proxy < "u" ? new Proxy(x2, { get: (a22, b) => (typeof __require < "u" ? __require : a22)[b] }) : x2)(function(x2) {
...
function printIteratorEntries(iterator, config3, indentation, depth, refs, printer22, separator = ": ") {
  ...
  if (!current.done) {
    ...
    for (; !current.done; ) {
      ...
      let name = printer22(current.value[0], config3, indentationNext, depth, refs), value = printer22(current.value[1], config3, indentationNext, depth, refs);
...
function printIteratorValues(iterator, config3, indentation, depth, refs, printer22) {
  let result = "", width = 0, current = iterator.next();
  if (!current.done) {
    result += config3.spacingOuter;
    let indentationNext = indentation + config3.indent;
    for (; !current.done; ) {
      ...
      result += printer22(current.value, config3, indentationNext, depth, refs), current = iterator.next(), current.done ? config3.min || (result += ",") : result += `,${config3.spacingInner}`;
    }
    result += config3.spacingOuter + indentation;
  }
  return result;
}
function printListItems(list, config3, indentation, depth, refs, printer22) {
  let result = "";
  ...
  if (length > 0) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      (isDataView(list) || i in list) && (result += printer22(isDataView(list) ? list.getInt8(i) : list[i], config3, indentationNext, depth, refs)), i < length - 1 ? result += `,${config3.spacingInner}` : config3.min || (result += ",");
    }
    result += config3.spacingOuter + indentation;
  }
  return result;
}
function printObjectProperties(val, config3, indentation, depth, refs, printer22) {
  let result = "", keys2 = getKeysOfEnumerableProperties(val, config3.compareKeys);
  if (keys2.length > 0) {
    result += config3.spacingOuter;
    let indentationNext = indentation + config3.indent;
    for (let i = 0; i < keys2.length; i++) {
      let key = keys2[i], name = printer22(key, config3, indentationNext, depth, refs), value = printer22(val[key], config3, indentationNext, depth, refs);
...
var SPACE$2 = " ";
var serialize$5 = (val, config3, indentation, depth, refs, printer22) => {
  let stringedValue = val.toString();
  if (stringedValue === "ArrayContaining" || stringedValue === "ArrayNotContaining") return ++depth > config3.maxDepth ? `[${stringedValue}]` : `${stringedValue + SPACE$2}[${printListItems(val.sample, config3, indentation, depth, refs, printer22)}]`;
  if (stringedValue === "ObjectContaining" || stringedValue === "ObjectNotContaining") return ++depth > config3.maxDepth ? `[${stringedValue}]` : `${stringedValue + SPACE$2}{${printObjectProperties(val.sample, config3, indentation, depth, refs, printer22)}}`;
  if (stringedValue === "StringMatching" || stringedValue === "StringNotMatching" || stringedValue === "StringContaining" || stringedValue === "StringNotContaining") return stringedValue + SPACE$2 + printer22(val.sample, config3, indentation, depth, refs);
...
var serialize$4 = (collection, config3, indentation, depth, refs, printer22) => {
  let name = collection.constructor.name;
  return ++depth > config3.maxDepth ? `[${name}]` : (config3.min ? "" : name + SPACE$1) + (OBJECT_NAMES.has(name) ? `{${printObjectProperties(isNamedNodeMap(collection) ? [...collection].reduce((props, attribute) => (props[attribute.name] = attribute.value, props), {}) : { ...collection }, config3, indentation, depth, refs, printer22)}}` : `[${printListItems([...collection], config3, indentation, depth, refs, printer22)}]`);
};
var plugin$4 = { serialize: serialize$4, test: test$4 };
function escapeHTML(str) {
  return str.replaceAll("<", "&lt;").replaceAll(">", "&gt;");
}
function printProps(keys2, props, config3, indentation, depth, refs, printer22) {
  let indentationNext = indentation + config3.indent, colors = config3.colors;
  return keys2.map((key) => {
    let value = props[key], printed = printer22(value, config3, indentationNext, depth, refs);
...
function printChildren(children, config3, indentation, depth, refs, printer22) {
  return children.map((child) => config3.spacingOuter + indentation + (typeof child == "string" ? printText(child, config3) : printer22(child, config3, indentation, depth, refs))).join("");
}
...
var serialize$3 = (node, config3, indentation, depth, refs, printer22) => {
  ...
  return ++depth > config3.maxDepth ? printElementAsLeaf(type5, config3) : printElement(type5, printProps(nodeIsFragment(node) ? [] : Array.from(node.attributes, (attr) => attr.name).sort(), nodeIsFragment(node) ? {} : [...node.attributes].reduce((props, attribute) => (props[attribute.name] = attribute.value, props), {}), config3, indentation + config3.indent, depth, refs, printer22), printChildren(Array.prototype.slice.call(node.childNodes || node.children), config3, indentation + config3.indent, depth, refs, printer22), config3, indentation);
};
...
var SPACE = " ";
var LAZY = "…";
function printImmutableEntries(val, config3, indentation, depth, refs, printer22, type5) {
  return ++depth > config3.maxDepth ? printAsLeaf(getImmutableName(type5)) : `${getImmutableName(type5) + SPACE}{${printIteratorEntries(val.entries(), config3, indentation, depth, refs, printer22)}}`;
}
...
function printImmutableRecord(val, config3, indentation, depth, refs, printer22) {
  let name = getImmutableName(val._name || "Record");
  return ++depth > config3.maxDepth ? printAsLeaf(name) : `${name + SPACE}{${printIteratorEntries(getRecordEntries(val), config3, indentation, depth, refs, printer22)}}`;
}
function printImmutableSeq(val, config3, indentation, depth, refs, printer22) {
  let name = getImmutableName("Seq");
  return ++depth > config3.maxDepth ? printAsLeaf(name) : val[IS_KEYED_SENTINEL] ? `${name + SPACE}{${val._iter || val._object ? printIteratorEntries(val.entries(), config3, indentation, depth, refs, printer22) : LAZY}}` : `${name + SPACE}[${val._iter || val._array || val._collection || val._iterable ? printIteratorValues(val.values(), config3, indentation, depth, refs, printer22) : LAZY}]`;
}
function printImmutableValues(val, config3, indentation, depth, refs, printer22, type5) {
  return ++depth > config3.maxDepth ? printAsLeaf(getImmutableName(type5)) : `${getImmutableName(type5) + SPACE}[${printIteratorValues(val.values(), config3, indentation, depth, refs, printer22)}]`;
}
var serialize$2 = (val, config3, indentation, depth, refs, printer22) => val[IS_MAP_SENTINEL] ? printImmutableEntries(val, config3, indentation, depth, refs, printer22, val[IS_ORDERED_SENTINEL] ? "OrderedMap" : "Map") : val[IS_LIST_SENTINEL] ? printImmutableValues(val, config3, indentation, depth, refs, printer22, "List") : val[IS_SET_SENTINEL] ? printImmutableValues(val, config3, indentation, depth, refs, printer22, val[IS_ORDERED_SENTINEL] ? "OrderedSet" : "Set") : val[IS_STACK_SENTINEL] ? printImmutableValues(val, config3, indentation, depth, refs, printer22, "Stack") : val[IS_SEQ_SENTINEL] ? printImmutableSeq(val, config3, indentation, depth, refs, printer22) : printImmutableRecord(val, config3, indentation, depth, refs, printer22);
...
var serialize$1 = (element, config3, indentation, depth, refs, printer22) => ++depth > config3.maxDepth ? printElementAsLeaf(getType(element), config3) : printElement(getType(element), printProps(getPropKeys$1(element), element.props, config3, indentation + config3.indent, depth, refs, printer22), printChildren(getChildren(element.props.children), config3, indentation + config3.indent, depth, refs, printer22), config3, indentation);
...
var serialize = (object, config3, indentation, depth, refs, printer22) => ++depth > config3.maxDepth ? printElementAsLeaf(object.type, config3) : printElement(object.type, object.props ? printProps(getPropKeys(object), object.props, config3, indentation + config3.indent, depth, refs, printer22) : "", object.children ? printChildren(object.children, config3, indentation + config3.indent, depth, refs, printer22) : "", config3, indentation);
...
function printPlugin(plugin22, val, config3, indentation, depth, refs) {
  let printed;
  try {
    printed = isNewPlugin(plugin22) ? plugin22.serialize(val, config3, indentation, depth, refs, printer) : plugin22.print(val, (valChild) => printer(valChild, config3, indentation, depth, refs), (str) => {
      let indentationNext = indentation + config3.indent;
...
function printer(val, config3, indentation, depth, refs, hasCalledToJSON) {
  ...
  if (plugin22 !== null) return printPlugin(plugin22, val, config3, indentation, depth, refs);
...
function format(val, options) {
  if (options && (validateOptions(options), options.plugins)) {
    let plugin22 = findPlugin(options.plugins, val);
    if (plugin22 !== null) return printPlugin(plugin22, val, getConfig(options), "", 0, []);
  }
...
function normaliseOptions({ showHidden = false, depth = 2, colors = false, customInspect = true, showProxy = false, maxArrayLength = 1 / 0, breakLength = 1 / 0, seen = [], truncate: truncate23 = 1 / 0, stylize = String } = {}, inspect33) {
  let options = { showHidden: !!showHidden, depth: Number(depth), colors: !!colors, customInspect: !!customInspect, showProxy: !!showProxy, maxArrayLength: Number(maxArrayLength), breakLength: Number(breakLength), truncate: Number(truncate23), seen, inspect: inspect33, stylize };
...
function inspectHTML(element, options) {
  ...
  let truncate23 = options.truncate, children = inspectHTMLCollection(element.children, options);
...
function format2(...args) {
  if (typeof args[0] != "string") {
    let objects = [];
    for (let i2 = 0; i2 < args.length; i2++) objects.push(inspect2(args[i2], { depth: 0, colors: false }));
    return objects.join(" ");
  }
  let len = args.length, i = 1, template = args[0], str = String(template).replace(formatRegExp, (x2) => {
    ...
    switch (x2) {
      case "%s": {
        let value = args[i++];
        return typeof value == "bigint" ? `${value.toString()}n` : typeof value == "number" && value === 0 && 1 / value < 0 ? "-0" : typeof value == "object" && value !== null ? inspect2(value, { depth: 0, colors: false }) : String(value);
      }
      case "%d": {
        let value = args[i++];
  ...
  for (let x2 = args[i]; i < len; x2 = args[++i]) x2 === null || typeof x2 != "object" ? str += ` ${x2}` : str += ` ${inspect2(x2)}`;
  return str;
}
...
function clone(val, seen, options = defaultCloneOptions) {
  ...
  if (Array.isArray(val)) {
    for (out = Array.from({ length: k2 = val.length }), seen.set(val, out); k2--; ) out[k2] = clone(val[k2], seen, options);
    return out;
  }
...
function joinAlignedDiffsExpand(diffs, options) {
  return diffs.map((diff22, i, diffs2) => {
    let line = diff22[1], isFirstOrLast = i === 0 || i === diffs2.length - 1;
    switch (diff22[0]) {
      case DIFF_DELETE:
...
function diffLinesRaw(aLines, bLines, options) {
  let truncate23 = (options == null ? void 0 : options.truncateThreshold) ?? false, truncateThreshold = Math.max(Math.floor((options == null ? void 0 : options.truncateThreshold) ?? 0), 0), aLength = truncate23 ? Math.min(aLines.length, truncateThreshold) : aLines.length, bLength = truncate23 ? Math.min(bLines.length, truncateThreshold) : bLines.length, truncated = aLength !== aLines.length || bLength !== bLines.length, isCommon = (aIndex2, bIndex2) => aLines[aIndex2] === bLines[bIndex2], diffs = [], aIndex = 0, bIndex = 0;
...
function diffStrings(a22, b, options) {
  let truncate23 = (options == null ? void 0 : options.truncateThreshold) ?? false, truncateThreshold = Math.max(Math.floor((options == null ? void 0 : options.truncateThreshold) ?? 0), 0), aLength = a22.length, bLength = b.length;
  if (truncate23) {
    let aMultipleLines = a22.includes(`
`), bMultipleLines = b.includes(`
`), aNewLineSymbol = getNewLineSymbol(a22), bNewLineSymbol = getNewLineSymbol(b), _a2 = aMultipleLines ? `${a22.split(aNewLineSymbol, truncateThreshold).join(aNewLineSymbol)}
...
function hasCommonDiff(diffs, isMultiline) {
  if (isMultiline) {
    ...
    return diffs.some((diff22, i) => diff22[0] === DIFF_EQUAL && (i !== iLast || diff22[1] !== `
`));
  }
...
function diff(a22, b, options) {
  ...
  if (expectedType !== getType3(b)) {
    let { aAnnotation, aColor, aIndicator, bAnnotation, bColor, bIndicator } = normalizeDiffOptions(options), formatOptions = getFormatOptions(FALLBACK_FORMAT_OPTIONS, options), aDisplay = format(a22, formatOptions), bDisplay = format(b, formatOptions), aDiff = `${aColor(`${aIndicator} ${aAnnotation}:`)} 
...
function getObjectsDifference(a22, b, formatOptions, options) {
  let formatOptionsZeroIndent = { ...formatOptions, indent: 0 }, aCompare = format(a22, formatOptionsZeroIndent), bCompare = format(b, formatOptionsZeroIndent);
...
function getCommonAndChangedSubstrings(diffs, op, hasCommonDiff22) {
  return diffs.reduce((reduced, diff22) => reduced + (diff22[0] === DIFF_EQUAL ? diff22[1] : diff22[0] === op ? hasCommonDiff22 ? f2.inverse(diff22[1]) : diff22[1] : ""), "");
}
...
var Instrumenter = class {
  constructor() {
    ...
    this.channel = import_preview_api.addons.getChannel(), this.state = ((_a2 = import_global.global.window) == null ? void 0 : _a2.parent.__STORYBOOK_ADDON_INTERACTIONS_INSTRUMENTER_STATE__) || {};
  ...
  setState(storyId, update) {
    ...
    this.state = { ...this.state, [storyId]: { ...state, ...patch } }, ((_a2 = import_global.global.window) == null ? void 0 : _a2.parent) && (import_global.global.window.parent.__STORYBOOK_ADDON_INTERACTIONS_INSTRUMENTER_STATE__ = this.state);
  }
  cleanup() {
    var _a2;
    ...
    this.channel.emit(EVENTS.SYNC, payload), ((_a2 = import_global.global.window) == null ? void 0 : _a2.parent) && (import_global.global.window.parent.__STORYBOOK_ADDON_INTERACTIONS_INSTRUMENTER_STATE__ = this.state);
  }
  getLog(storyId) {
  ...
  track(method, fn3, object, args, options) {
    var _a2, _b, _c, _d;
    let storyId = ((_a2 = args == null ? void 0 : args[0]) == null ? void 0 : _a2.__storyId__) || ((_d = (_c = (_b = import_global.global.__STORYBOOK_PREVIEW__) == null ? void 0 : _b.selectionStore) == null ? void 0 : _c.selection) == null ? void 0 : _d.storyId), { cursor, ancestors } = this.getState(storyId);
  ...
  invoke(fn3, object, call2, options) {
    let { callRefsByResult, renderPhase } = this.getState(call2.storyId), maximumDepth = 25, serializeValues = (value, depth, seen) => {
      ...
      if (value instanceof ((_a2 = import_global.global.window) == null ? void 0 : _a2.HTMLElement)) {
    ...
    let handleException = (e) => {
      if (e instanceof Error) {
        let { name, message, stack, callId = call2.id } = e, { showDiff = void 0, diff: diff22 = void 0, actual = void 0, expected = void 0 } = e.name === "AssertionError" ? processError(e) : e, exception = { name, message, stack, callId, showDiff, diff: diff22, actual, expected };
  ...
  update(call2) {
    this.channel.emit(EVENTS.CALL, call2), this.setState(call2.storyId, ({ calls }) => {
      let callsById = calls.concat(call2).reduce((a22, c) => Object.assign(a22, { [c.id]: c }), {});
      return { calls: Object.values(callsById).sort((a22, b) => a22.id.localeCompare(b.id, void 0, { numeric: true })) };
...
var __require3 = ((x2) => typeof __require < "u" ? __require : typeof Proxy < "u" ? new Proxy(x2, { get: (a22, b) => (typeof __require < "u" ? __require : a22)[b] }) : x2)(function(x2) {
  if (typeof __require < "u") return __require.apply(this, arguments);
  throw Error('Dynamic require of "' + x2 + '" is not supported');
});
...
var require_conversions = __commonJS({ "../../node_modules/color-convert/conversions.js"(exports, module2) {
  ...
  convert.rgb.hsl = function(rgb) {
    let r = rgb[0] / 255, g3 = rgb[1] / 255, b = rgb[2] / 255, min = Math.min(r, g3, b), max = Math.max(r, g3, b), delta = max - min, h22, s;
    max === min ? h22 = 0 : r === max ? h22 = (g3 - b) / delta : g3 === max ? h22 = 2 + (b - r) / delta : b === max && (h22 = 4 + (r - g3) / delta), h22 = Math.min(h22 * 60, 360), h22 < 0 && (h22 += 360);
    let l = (min + max) / 2;
    return max === min ? s = 0 : l <= 0.5 ? s = delta / (max + min) : s = delta / (2 - max - min), [h22, s * 100, l * 100];
  };
  convert.rgb.hsv = function(rgb) {
    let rdif, gdif, bdif, h22, s, r = rgb[0] / 255, g3 = rgb[1] / 255, b = rgb[2] / 255, v = Math.max(r, g3, b), diff22 = v - Math.min(r, g3, b), diffc = function(c) {
      return (v - c) / 6 / diff22 + 1 / 2;
    };
    return diff22 === 0 ? (h22 = 0, s = 0) : (s = diff22 / v, rdif = diffc(r), gdif = diffc(g3), bdif = diffc(b), r === v ? h22 = bdif - gdif : g3 === v ? h22 = 1 / 3 + rdif - bdif : b === v && (h22 = 2 / 3 + gdif - rdif), h22 < 0 ? h22 += 1 : h22 > 1 && (h22 -= 1)), [h22 * 360, s * 100, v * 100];
  };
  convert.rgb.hwb = function(rgb) {
    let r = rgb[0], g3 = rgb[1], b = rgb[2], h22 = convert.rgb.hsl(rgb)[0], w2 = 1 / 255 * Math.min(r, Math.min(g3, b));
    return b = 1 - 1 / 255 * Math.max(r, Math.max(g3, b)), [h22, w2 * 100, b * 100];
  };
  ...
  convert.rgb.lab = function(rgb) {
    ...
    let l = 116 * y2 - 16, a22 = 500 * (x2 - y2), b = 200 * (y2 - z);
    return [l, a22, b];
  };
  convert.hsl.rgb = function(hsl) {
    let h22 = hsl[0] / 360, s = hsl[1] / 100, l = hsl[2] / 100, t2, t3, val;
    if (s === 0) return val = l * 255, [val, val, val];
    l < 0.5 ? t2 = l * (1 + s) : t2 = l + s - l * s;
    let t1 = 2 * l - t2, rgb = [0, 0, 0];
    for (let i = 0; i < 3; i++) t3 = h22 + 1 / 3 * -(i - 1), t3 < 0 && t3++, t3 > 1 && t3--, 6 * t3 < 1 ? val = t1 + (t2 - t1) * 6 * t3 : 2 * t3 < 1 ? val = t2 : 3 * t3 < 2 ? val = t1 + (t2 - t1) * (2 / 3 - t3) * 6 : val = t1, rgb[i] = val * 255;
    return rgb;
  };
  convert.hsl.hsv = function(hsl) {
    let h22 = hsl[0], s = hsl[1] / 100, l = hsl[2] / 100, smin = s, lmin = Math.max(l, 0.01);
    l *= 2, s *= l <= 1 ? l : 2 - l, smin *= lmin <= 1 ? lmin : 2 - lmin;
    let v = (l + s) / 2, sv = l === 0 ? 2 * smin / (lmin + smin) : 2 * s / (l + s);
    return [h22, sv * 100, v * 100];
  };
  convert.hsv.rgb = function(hsv) {
    let h22 = hsv[0] / 60, s = hsv[1] / 100, v = hsv[2] / 100, hi = Math.floor(h22) % 6, f4 = h22 - Math.floor(h22), p3 = 255 * v * (1 - s), q = 255 * v * (1 - s * f4), t = 255 * v * (1 - s * (1 - f4));
    switch (v *= 255, hi) {
      case 0:
        return [v, t, p3];
      case 1:
  ...
  convert.hsv.hsl = function(hsv) {
    let h22 = hsv[0], s = hsv[1] / 100, v = hsv[2] / 100, vmin = Math.max(v, 0.01), sl, l;
    l = (2 - s) * v;
    let lmin = (2 - s) * vmin;
    return sl = s * vmin, sl /= lmin <= 1 ? lmin : 2 - lmin, sl = sl || 0, l /= 2, [h22, sl * 100, l * 100];
  };
  convert.hwb.rgb = function(hwb) {
    let h22 = hwb[0] / 360, wh = hwb[1] / 100, bl = hwb[2] / 100, ratio = wh + bl, f4;
    ratio > 1 && (wh /= ratio, bl /= ratio);
    let i = Math.floor(6 * h22), v = 1 - bl;
    f4 = 6 * h22 - i, i & 1 && (f4 = 1 - f4);
  ...
  convert.lab.xyz = function(lab) {
    let l = lab[0], a22 = lab[1], b = lab[2], x2, y2, z;
    y2 = (l + 16) / 116, x2 = a22 / 500 + y2, z = y2 - b / 200;
  ...
  convert.lab.lch = function(lab) {
    let l = lab[0], a22 = lab[1], b = lab[2], h22;
    h22 = Math.atan2(b, a22) * 360 / 2 / Math.PI, h22 < 0 && (h22 += 360);
    let c = Math.sqrt(a22 * a22 + b * b);
    return [l, c, h22];
  };
  convert.lch.lab = function(lch) {
    let l = lch[0], c = lch[1], hr = lch[2] / 360 * 2 * Math.PI, a22 = c * Math.cos(hr), b = c * Math.sin(hr);
    return [l, a22, b];
  };
  ...
  convert.hcg.rgb = function(hcg) {
    let h22 = hcg[0] / 360, c = hcg[1] / 100, g3 = hcg[2] / 100;
    if (c === 0) return [g3 * 255, g3 * 255, g3 * 255];
    let pure = [0, 0, 0], hi = h22 % 1 * 6, v = hi % 1, w2 = 1 - v, mg = 0;
...
var require_isFunction = __commonJS({ "../../node_modules/lodash/isFunction.js"(exports, module2) {
  ...
  function isFunction(value) {
    if (!isObject22(value)) return false;
...
var require_baseIsNative = __commonJS({ "../../node_modules/lodash/_baseIsNative.js"(exports, module2) {
  ...
  function baseIsNative(value) {
    if (!isObject22(value) || isMasked(value)) return false;
...
var require_collections = __commonJS({ "../../node_modules/pretty-format/build/collections.js"(exports) {
  ...
  function printIteratorEntries22(iterator, config3, indentation, depth, refs, printer22, separator = ": ") {
  ...
  function printIteratorValues22(iterator, config3, indentation, depth, refs, printer22) {
    let result = "", current = iterator.next();
    if (!current.done) {
      result += config3.spacingOuter;
      let indentationNext = indentation + config3.indent;
      for (; !current.done; ) result += indentationNext + printer22(current.value, config3, indentationNext, depth, refs), current = iterator.next(), current.done ? config3.min || (result += ",") : result += "," + config3.spacingInner;
      result += config3.spacingOuter + indentation;
    }
    return result;
  }
  function printListItems22(list, config3, indentation, depth, refs, printer22) {
    let result = "";
    if (list.length) {
      result += config3.spacingOuter;
      let indentationNext = indentation + config3.indent;
      for (let i = 0; i < list.length; i++) result += indentationNext, i in list && (result += printer22(list[i], config3, indentationNext, depth, refs)), i < list.length - 1 ? result += "," + config3.spacingInner : config3.min || (result += ",");
      result += config3.spacingOuter + indentation;
    }
    return result;
  }
  function printObjectProperties22(val, config3, indentation, depth, refs, printer22) {
    let result = "", keys2 = getKeysOfEnumerableProperties22(val, config3.compareKeys);
    if (keys2.length) {
      result += config3.spacingOuter;
      let indentationNext = indentation + config3.indent;
      for (let i = 0; i < keys2.length; i++) {
...
var require_AsymmetricMatcher = __commonJS({ "../../node_modules/pretty-format/build/plugins/AsymmetricMatcher.js"(exports) {
  ...
  }(), Symbol2 = global3["jest-symbol-do-not-touch"] || global3.Symbol, asymmetricMatcher22 = typeof Symbol2 == "function" && Symbol2.for ? Symbol2.for("jest.asymmetricMatcher") : 1267621, SPACE22 = " ", serialize22 = (val, config3, indentation, depth, refs, printer22) => {
    let stringedValue = val.toString();
    return stringedValue === "ArrayContaining" || stringedValue === "ArrayNotContaining" ? ++depth > config3.maxDepth ? "[" + stringedValue + "]" : stringedValue + SPACE22 + "[" + (0, _collections.printListItems)(val.sample, config3, indentation, depth, refs, printer22) + "]" : stringedValue === "ObjectContaining" || stringedValue === "ObjectNotContaining" ? ++depth > config3.maxDepth ? "[" + stringedValue + "]" : stringedValue + SPACE22 + "{" + (0, _collections.printObjectProperties)(val.sample, config3, indentation, depth, refs, printer22) + "}" : stringedValue === "StringMatching" || stringedValue === "StringNotMatching" || stringedValue === "StringContaining" || stringedValue === "StringNotContaining" ? stringedValue + SPACE22 + printer22(val.sample, config3, indentation, depth, refs) : val.toAsymmetricMatcher();
  };
...
var require_ConvertAnsi = __commonJS({ "../../node_modules/pretty-format/build/plugins/ConvertAnsi.js"(exports) {
  ...
  exports.test = test3;
  var serialize22 = (val, config3, indentation, depth, refs, printer22) => printer22(toHumanReadableAnsi(val), config3, indentation, depth, refs);
  exports.serialize = serialize22;
  var plugin22 = { serialize: serialize22, test: test3 }, _default2 = plugin22;
  exports.default = _default2;
} });
var require_DOMCollection = __commonJS({ "../../node_modules/pretty-format/build/plugins/DOMCollection.js"(exports) {
  Object.defineProperty(exports, "__esModule", { value: true });
  exports.test = exports.serialize = exports.default = void 0;
  var _collections = require_collections(), SPACE22 = " ", OBJECT_NAMES22 = ["DOMStringMap", "NamedNodeMap"], ARRAY_REGEXP22 = /^(HTML\w*Collection|NodeList)$/, testName22 = (name) => OBJECT_NAMES22.indexOf(name) !== -1 || ARRAY_REGEXP22.test(name), test3 = (val) => val && val.constructor && !!val.constructor.name && testName22(val.constructor.name);
  exports.test = test3;
  var isNamedNodeMap22 = (collection) => collection.constructor.name === "NamedNodeMap", serialize22 = (collection, config3, indentation, depth, refs, printer22) => {
    let name = collection.constructor.name;
    return ++depth > config3.maxDepth ? "[" + name + "]" : (config3.min ? "" : name + SPACE22) + (OBJECT_NAMES22.indexOf(name) !== -1 ? "{" + (0, _collections.printObjectProperties)(isNamedNodeMap22(collection) ? Array.from(collection).reduce((props, attribute) => (props[attribute.name] = attribute.value, props), {}) : { ...collection }, config3, indentation, depth, refs, printer22) + "}" : "[" + (0, _collections.printListItems)(Array.from(collection), config3, indentation, depth, refs, printer22) + "]");
  };
  exports.serialize = serialize22;
  var plugin22 = { serialize: serialize22, test: test3 }, _default2 = plugin22;
  exports.default = _default2;
...
var require_markup = __commonJS({ "../../node_modules/pretty-format/build/plugins/lib/markup.js"(exports) {
  ...
  var printProps3 = (keys2, props, config3, indentation, depth, refs, printer22) => {
    let indentationNext = indentation + config3.indent, colors = config3.colors;
    return keys2.map((key) => {
      let value = props[key], printed = printer22(value, config3, indentationNext, depth, refs);
...
`) !== -1 && (printed = config3.spacingOuter + indentationNext + printed + config3.spacingOuter + indentation), printed = "{" + printed + "}"), config3.spacingInner + indentation + colors.prop.open + key + colors.prop.close + "=" + colors.value.open + printed + colors.value.close;
  ...
  var printChildren3 = (children, config3, indentation, depth, refs, printer22) => children.map((child) => config3.spacingOuter + indentation + (typeof child == "string" ? printText3(child, config3) : printer22(child, config3, indentation, depth, refs))).join("");
...
var require_DOMElement = __commonJS({ "../../node_modules/pretty-format/build/plugins/DOMElement.js"(exports) {
  ...
  var serialize22 = (node, config3, indentation, depth, refs, printer22) => {
...
var require_Immutable = __commonJS({ "../../node_modules/pretty-format/build/plugins/Immutable.js"(exports) {
  Object.defineProperty(exports, "__esModule", { value: true });
  exports.test = exports.serialize = exports.default = void 0;
  var _collections = require_collections(), IS_ITERABLE_SENTINEL22 = "@@__IMMUTABLE_ITERABLE__@@", IS_LIST_SENTINEL3 = "@@__IMMUTABLE_LIST__@@", IS_KEYED_SENTINEL3 = "@@__IMMUTABLE_KEYED__@@", IS_MAP_SENTINEL22 = "@@__IMMUTABLE_MAP__@@", IS_ORDERED_SENTINEL3 = "@@__IMMUTABLE_ORDERED__@@", IS_RECORD_SENTINEL22 = "@@__IMMUTABLE_RECORD__@@", IS_SEQ_SENTINEL22 = "@@__IMMUTABLE_SEQ__@@", IS_SET_SENTINEL3 = "@@__IMMUTABLE_SET__@@", IS_STACK_SENTINEL22 = "@@__IMMUTABLE_STACK__@@", getImmutableName22 = (name) => "Immutable." + name, printAsLeaf22 = (name) => "[" + name + "]", SPACE22 = " ", LAZY22 = "…", printImmutableEntries22 = (val, config3, indentation, depth, refs, printer22, type5) => ++depth > config3.maxDepth ? printAsLeaf22(getImmutableName22(type5)) : getImmutableName22(type5) + SPACE22 + "{" + (0, _collections.printIteratorEntries)(val.entries(), config3, indentation, depth, refs, printer22) + "}";
  ...
  var printImmutableRecord22 = (val, config3, indentation, depth, refs, printer22) => {
    let name = getImmutableName22(val._name || "Record");
    return ++depth > config3.maxDepth ? printAsLeaf22(name) : name + SPACE22 + "{" + (0, _collections.printIteratorEntries)(getRecordEntries22(val), config3, indentation, depth, refs, printer22) + "}";
  }, printImmutableSeq22 = (val, config3, indentation, depth, refs, printer22) => {
    let name = getImmutableName22("Seq");
    return ++depth > config3.maxDepth ? printAsLeaf22(name) : val[IS_KEYED_SENTINEL3] ? name + SPACE22 + "{" + (val._iter || val._object ? (0, _collections.printIteratorEntries)(val.entries(), config3, indentation, depth, refs, printer22) : LAZY22) + "}" : name + SPACE22 + "[" + (val._iter || val._array || val._collection || val._iterable ? (0, _collections.printIteratorValues)(val.values(), config3, indentation, depth, refs, printer22) : LAZY22) + "]";
  }, printImmutableValues22 = (val, config3, indentation, depth, refs, printer22, type5) => ++depth > config3.maxDepth ? printAsLeaf22(getImmutableName22(type5)) : getImmutableName22(type5) + SPACE22 + "[" + (0, _collections.printIteratorValues)(val.values(), config3, indentation, depth, refs, printer22) + "]", serialize22 = (val, config3, indentation, depth, refs, printer22) => val[IS_MAP_SENTINEL22] ? printImmutableEntries22(val, config3, indentation, depth, refs, printer22, val[IS_ORDERED_SENTINEL3] ? "OrderedMap" : "Map") : val[IS_LIST_SENTINEL3] ? printImmutableValues22(val, config3, indentation, depth, refs, printer22, "List") : val[IS_SET_SENTINEL3] ? printImmutableValues22(val, config3, indentation, depth, refs, printer22, val[IS_ORDERED_SENTINEL3] ? "OrderedSet" : "Set") : val[IS_STACK_SENTINEL22] ? printImmutableValues22(val, config3, indentation, depth, refs, printer22, "Stack") : val[IS_SEQ_SENTINEL22] ? printImmutableSeq22(val, config3, indentation, depth, refs, printer22) : printImmutableRecord22(val, config3, indentation, depth, refs, printer22);
  exports.serialize = serialize22;
...
var require_ReactElement = __commonJS({ "../../node_modules/pretty-format/build/plugins/ReactElement.js"(exports) {
  ...
  var getChildren22 = (arg, children = []) => (Array.isArray(arg) ? arg.forEach((item) => {
    ...
    let { props } = element;
    return Object.keys(props).filter((key) => key !== "children" && props[key] !== void 0).sort();
  }, serialize22 = (element, config3, indentation, depth, refs, printer22) => ++depth > config3.maxDepth ? (0, _markup.printElementAsLeaf)(getType42(element), config3) : (0, _markup.printElement)(getType42(element), (0, _markup.printProps)(getPropKeys22(element), element.props, config3, indentation + config3.indent, depth, refs, printer22), (0, _markup.printChildren)(getChildren22(element.props.children), config3, indentation + config3.indent, depth, refs, printer22), config3, indentation);
  exports.serialize = serialize22;
...
var require_ReactTestComponent = __commonJS({ "../../node_modules/pretty-format/build/plugins/ReactTestComponent.js"(exports) {
  ...
  var _markup = require_markup(), global3 = function() {
    ...
    let { props } = object;
    return props ? Object.keys(props).filter((key) => props[key] !== void 0).sort() : [];
  }, serialize22 = (object, config3, indentation, depth, refs, printer22) => ++depth > config3.maxDepth ? (0, _markup.printElementAsLeaf)(object.type, config3) : (0, _markup.printElement)(object.type, object.props ? (0, _markup.printProps)(getPropKeys22(object), object.props, config3, indentation + config3.indent, depth, refs, printer22) : "", object.children ? (0, _markup.printChildren)(object.children, config3, indentation + config3.indent, depth, refs, printer22) : "", config3, indentation);
  exports.serialize = serialize22;
...
var require_build = __commonJS({ "../../node_modules/pretty-format/build/index.js"(exports) {
  ...
  function printComplexValue22(val, config3, indentation, depth, refs, hasCalledToJSON) {
    if (refs.indexOf(val) !== -1) return "[Circular]";
    refs = refs.slice(), refs.push(val);
    let hitMaxDepth = ++depth > config3.maxDepth, min = config3.min;
    if (config3.callToJSON && !hitMaxDepth && val.toJSON && typeof val.toJSON == "function" && !hasCalledToJSON) return printer22(val.toJSON(), config3, indentation, depth, refs, true);
    let toStringed = toString4.call(val);
    return toStringed === "[object Arguments]" ? hitMaxDepth ? "[Arguments]" : (min ? "" : "Arguments ") + "[" + (0, _collections.printListItems)(val, config3, indentation, depth, refs, printer22) + "]" : isToStringedArrayType22(toStringed) ? hitMaxDepth ? "[" + val.constructor.name + "]" : (min || !config3.printBasicPrototype && val.constructor.name === "Array" ? "" : val.constructor.name + " ") + "[" + (0, _collections.printListItems)(val, config3, indentation, depth, refs, printer22) + "]" : toStringed === "[object Map]" ? hitMaxDepth ? "[Map]" : "Map {" + (0, _collections.printIteratorEntries)(val.entries(), config3, indentation, depth, refs, printer22, " => ") + "}" : toStringed === "[object Set]" ? hitMaxDepth ? "[Set]" : "Set {" + (0, _collections.printIteratorValues)(val.values(), config3, indentation, depth, refs, printer22) + "}" : hitMaxDepth || isWindow22(val) ? "[" + getConstructorName3(val) + "]" : (min || !config3.printBasicPrototype && getConstructorName3(val) === "Object" ? "" : getConstructorName3(val) + " ") + "{" + (0, _collections.printObjectProperties)(val, config3, indentation, depth, refs, printer22) + "}";
  }
  function isNewPlugin22(plugin22) {
    return plugin22.serialize != null;
  }
  function printPlugin22(plugin22, val, config3, indentation, depth, refs) {
    let printed;
    try {
      printed = isNewPlugin22(plugin22) ? plugin22.serialize(val, config3, indentation, depth, refs, printer22) : plugin22.print(val, (valChild) => printer22(valChild, config3, indentation, depth, refs), (str) => {
        let indentationNext = indentation + config3.indent;
...
` + indentationNext);
  ...
  function printer22(val, config3, indentation, depth, refs, hasCalledToJSON) {
    let plugin22 = findPlugin22(config3.plugins, val);
    if (plugin22 !== null) return printPlugin22(plugin22, val, config3, indentation, depth, refs);
    let basicResult = printBasicValue22(val, config3.printFunctionName, config3.escapeRegex, config3.escapeString);
    return basicResult !== null ? basicResult : printComplexValue22(val, config3, indentation, depth, refs, hasCalledToJSON);
  }
  ...
  function validateOptions22(options) {
    if (Object.keys(options).forEach((key) => {
      if (!DEFAULT_OPTIONS22.hasOwnProperty(key)) throw new Error(`pretty-format: Unknown option "${key}".`);
  ...
  var getColorsHighlight22 = (options) => DEFAULT_THEME_KEYS22.reduce((colors, key) => {
    let value = options.theme && options.theme[key] !== void 0 ? options.theme[key] : DEFAULT_THEME22[key], color = value && _ansiStyles.default[value];
    if (color && typeof color.close == "string" && typeof color.open == "string") colors[key] = color;
    else throw new Error(`pretty-format: Option "theme" has a key "${key}" whose value "${value}" is undefined in ansi-styles.`);
    return colors;
  }, /* @__PURE__ */ Object.create(null)), getColorsEmpty22 = () => DEFAULT_THEME_KEYS22.reduce((colors, key) => (colors[key] = { close: "", open: "" }, colors), /* @__PURE__ */ Object.create(null)), getPrintFunctionName22 = (options) => options && options.printFunctionName !== void 0 ? options.printFunctionName : DEFAULT_OPTIONS22.printFunctionName, getEscapeRegex22 = (options) => options && options.escapeRegex !== void 0 ? options.escapeRegex : DEFAULT_OPTIONS22.escapeRegex, getEscapeString22 = (options) => options && options.escapeString !== void 0 ? options.escapeString : DEFAULT_OPTIONS22.escapeString, getConfig4 = (options) => {
    var _options$printBasicPr;
    return { callToJSON: options && options.callToJSON !== void 0 ? options.callToJSON : DEFAULT_OPTIONS22.callToJSON, colors: options && options.highlight ? getColorsHighlight22(options) : getColorsEmpty22(), compareKeys: options && typeof options.compareKeys == "function" ? options.compareKeys : DEFAULT_OPTIONS22.compareKeys, escapeRegex: getEscapeRegex22(options), escapeString: getEscapeString22(options), indent: options && options.min ? "" : createIndent22(options && options.indent !== void 0 ? options.indent : DEFAULT_OPTIONS22.indent), maxDepth: options && options.maxDepth !== void 0 ? options.maxDepth : DEFAULT_OPTIONS22.maxDepth, min: options && options.min !== void 0 ? options.min : DEFAULT_OPTIONS22.min, plugins: options && options.plugins !== void 0 ? options.plugins : DEFAULT_OPTIONS22.plugins, printBasicPrototype: (_options$printBasicPr = options == null ? void 0 : options.printBasicPrototype) !== null && _options$printBasicPr !== void 0 ? _options$printBasicPr : true, printFunctionName: getPrintFunctionName22(options), spacingInner: options && options.min ? " " : `
...
` };
  ...
  function format4(val, options) {
    if (options && (validateOptions22(options), options.plugins)) {
      let plugin22 = findPlugin22(options.plugins, val);
      if (plugin22 !== null) return printPlugin22(plugin22, val, getConfig4(options), "", 0, []);
    }
    let basicResult = printBasicValue22(val, getPrintFunctionName22(options), getEscapeRegex22(options), getEscapeString22(options));
    return basicResult !== null ? basicResult : printComplexValue22(val, getConfig4(options), "", 0, []);
  }
...
__export2(check_error_exports, { compatibleConstructor: () => compatibleConstructor, compatibleInstance: () => compatibleInstance, compatibleMessage: () => compatibleMessage, getConstructorName: () => getConstructorName2, getMessage: () => getMessage });
...
function normaliseOptions2({ showHidden = false, depth = 2, colors = false, customInspect = true, showProxy = false, maxArrayLength = 1 / 0, breakLength = 1 / 0, seen = [], truncate: truncate222 = 1 / 0, stylize = String } = {}, inspect322) {
  let options = { showHidden: !!showHidden, depth: Number(depth), colors: !!colors, customInspect: !!customInspect, showProxy: !!showProxy, maxArrayLength: Number(maxArrayLength), breakLength: Number(breakLength), truncate: Number(truncate222), seen, inspect: inspect322, stylize };
...
try {
  let { getPromiseDetails, kPending, kRejected } = process.binding("util");
  Array.isArray(getPromiseDetails(Promise.resolve())) && (getPromiseValue2 = __name((value, options) => {
    let [state, innerValue] = getPromiseDetails(value);
    return state === kPending ? "Promise{<pending>}" : `Promise${state === kRejected ? "!" : ""}{${options.inspect(innerValue, options)}}`;
  }, "getPromiseValue"));
} catch {
...
function inspectHTML2(element, options) {
  ...
  let truncate222 = options.truncate, children = inspectHTMLCollection2(element.children, options);
...
function inspect3(value, opts = {}) {
  ...
  if (type32 === "object" && (type32 = toString3.call(value).slice(8, -1)), type32 in baseTypesMap2) return baseTypesMap2[type32](value, options);
  if (customInspect && value) {
...
function assertProperty(name, val, msg) {
  ...
  var hasProperty2;
  isOwn ? hasProperty2 = Object.prototype.hasOwnProperty.call(obj, name) : isNested ? hasProperty2 = pathInfo.exists : hasProperty2 = hasProperty(obj, name), (!negate || arguments.length === 1) && this.assert(hasProperty2, "expected #{this} to have " + descriptor + inspect22(name), "expected #{this} to not have " + descriptor + inspect22(name)), arguments.length > 1 && this.assert(hasProperty2 && isEql(val, value), "expected #{this} to have " + descriptor + inspect22(name) + " of #{exp}, but got #{act}", "expected #{this} to not have " + descriptor + inspect22(name) + " of #{act}", val, value), flag2(this, "object", value);
}
...
var import_redent = __toESM2(require_redent(), 1);
...
function arrayFrom(arrayLike, mapFn) {
  ...
  for (var len = toLength(items.length), A = isCallable(C3) ? Object(new C3(len)) : new Array(len), k2 = 0, kValue; k2 < len; ) kValue = items[k2], mapFn ? A[k2] = mapFn(kValue, k2) : A[k2] = kValue, k2 += 1;
...
var import_aria_query = __toESM2(require_lib(), 1);
var import_chalk = __toESM2(require_source(), 1);
var import_isEqualWith = __toESM2(require_isEqualWith(), 1);
var import_css = __toESM2(require_css_escape(), 1);
...
function printAttribute(stringify22, name, value) {
  return value === void 0 ? name : `${name}=${stringify22(value)}`;
}
function getAttributeComment(stringify22, name, value) {
  return value === void 0 ? `element.hasAttribute(${stringify22(name)})` : `element.getAttribute(${stringify22(name)}) === ${stringify22(value)}`;
}
...
__toESM2(require_redent(), 1);
__toESM2(require_lib(), 1);
__toESM2(require_source(), 1);
__toESM2(require_isEqualWith(), 1);
__toESM2(require_css_escape(), 1);
...
function deepClone2(val, options = defaultCloneOptions2) {
  return clone2(val, /* @__PURE__ */ new WeakMap(), options);
}
function clone2(val, seen, options = defaultCloneOptions2) {
  let k2, out;
  if (seen.has(val)) return seen.get(val);
  if (Array.isArray(val)) {
    for (out = Array(k2 = val.length), seen.set(val, out); k2--; ) out[k2] = clone2(val[k2], seen, options);
    return out;
  }
  if (Object.prototype.toString.call(val) === "[object Object]") {
    out = Object.create(Object.getPrototypeOf(val)), seen.set(val, out);
    ...
    for (let k22 of props) {
      let descriptor = Object.getOwnPropertyDescriptor(val, k22);
      if (!descriptor) continue;
      let cloned = clone2(val[k22], seen, options);
      options.forceWritable ? Object.defineProperty(out, k22, { enumerable: descriptor.enumerable, configurable: true, writable: true, value: cloned }) : "get" in descriptor ? Object.defineProperty(out, k22, { ...descriptor, get() {
        return cloned;
      } }) : Object.defineProperty(out, k22, { ...descriptor, value: cloned });
    }
...
var truncator22 = "…";
function colorise22(value, styleType) {
...
function normaliseOptions22({ showHidden = false, depth = 2, colors = false, customInspect = true, showProxy = false, maxArrayLength = 1 / 0, breakLength = 1 / 0, seen = [], truncate: truncate3 = 1 / 0, stylize = String } = {}, inspect5) {
  ...
  return options.colors && (options.stylize = colorise22), options;
}
...
function inspectList22(list, options, inspectItem, separator = ", ") {
  inspectItem = inspectItem || options.inspect;
  let size = list.length;
  if (size === 0) return "";
  let originalLength = options.truncate, output = "", peek = "", truncated = "";
  for (let i = 0; i < size; i += 1) {
    let last = i + 1 === list.length, secondToLast = i + 2 === list.length;
    truncated = `${truncator22}(${list.length - i})`;
    let value = list[i];
    options.truncate = originalLength - output.length - (last ? 0 : separator.length);
    let string = peek || inspectItem(value, options) + (last ? "" : separator), nextLength = output.length + string.length, truncatedLength = nextLength + truncated.length;
    if (last && nextLength > originalLength && output.length + truncated.length <= originalLength || !last && !secondToLast && truncatedLength > originalLength || (peek = last ? "" : inspectItem(list[i + 1], options) + (secondToLast ? "" : separator), !last && secondToLast && truncatedLength > originalLength && nextLength + peek.length > originalLength)) break;
    if (output += string, !last && !secondToLast && nextLength + peek.length >= originalLength) {
      truncated = `${truncator22}(${list.length - i - 1})`;
      break;
    }
    truncated = "";
  }
...
function quoteComplexKey22(key) {
  return key.match(/^[a-zA-Z_][a-zA-Z_0-9]*$/) ? key : JSON.stringify(key).replace(/'/g, "\\'").replace(/\\"/g, '"').replace(/(^"|"$)/g, "'");
}
function inspectProperty22([key, value], options) {
  return options.truncate -= 2, typeof key == "string" ? key = quoteComplexKey22(key) : typeof key != "number" && (key = `[${options.inspect(key, options)}]`), options.truncate -= key.length, value = options.inspect(value, options), `${key}: ${value}`;
}
function inspectArray22(array, options) {
  let nonIndexProperties = Object.keys(array).slice(array.length);
  if (!array.length && !nonIndexProperties.length) return "[]";
  options.truncate -= 4;
  let listContents = inspectList22(array, options);
  options.truncate -= listContents.length;
  let propertyContents = "";
  return nonIndexProperties.length && (propertyContents = inspectList22(nonIndexProperties.map((key) => [key, array[key]]), options, inspectProperty22)), `[ ${listContents}${propertyContents ? `, ${propertyContents}` : ""} ]`;
}
...
function inspectTypedArray22(array, options) {
  let name = getArrayName22(array);
  options.truncate -= name.length + 4;
  let nonIndexProperties = Object.keys(array).slice(array.length);
  if (!array.length && !nonIndexProperties.length) return `${name}[]`;
  let output = "";
  for (let i = 0; i < array.length; i++) {
    let string = `${options.stylize(truncate22(array[i], options.truncate), "number")}${i === array.length - 1 ? "" : ", "}`;
    if (options.truncate -= string.length, array[i] !== array.length && options.truncate <= 3) {
      output += `${truncator22}(${array.length - array[i] + 1})`;
      break;
    }
    output += string;
  }
  let propertyContents = "";
  return nonIndexProperties.length && (propertyContents = inspectList22(nonIndexProperties.map((key) => [key, array[key]]), options, inspectProperty22)), `${name}[ ${output}${propertyContents ? `, ${propertyContents}` : ""} ]`;
}
function inspectDate22(dateObject, options) {
  let stringRepresentation = dateObject.toJSON();
  if (stringRepresentation === null) return "Invalid Date";
  let split = stringRepresentation.split("T"), date = split[0];
  return options.stylize(`${date}T${truncate22(split[1], options.truncate - date.length - 1)}`, "date");
}
function inspectFunction22(func, options) {
  let functionType = func[Symbol.toStringTag] || "Function", name = func.name;
  return name ? options.stylize(`[${functionType} ${truncate22(name, options.truncate - 11)}]`, "special") : options.stylize(`[${functionType}]`, "special");
}
function inspectMapEntry22([key, value], options) {
  return options.truncate -= 4, key = options.inspect(key, options), options.truncate -= key.length, value = options.inspect(value, options), `${key} => ${value}`;
}
...
function inspectMap22(map, options) {
  return map.size - 1 <= 0 ? "Map{}" : (options.truncate -= 7, `Map{ ${inspectList22(mapToEntries22(map), options, inspectMapEntry22)} }`);
}
var isNaN3 = Number.isNaN || ((i) => i !== i);
function inspectNumber22(number, options) {
  return isNaN3(number) ? options.stylize("NaN", "number") : number === 1 / 0 ? options.stylize("Infinity", "number") : number === -1 / 0 ? options.stylize("-Infinity", "number") : number === 0 ? options.stylize(1 / number === 1 / 0 ? "+0" : "-0", "number") : options.stylize(truncate22(String(number), options.truncate), "number");
}
function inspectBigInt22(number, options) {
  let nums = truncate22(number.toString(), options.truncate - 1);
  return nums !== truncator22 && (nums += "n"), options.stylize(nums, "bigint");
}
function inspectRegExp22(value, options) {
  let flags = value.toString().split("/")[2], sourceLength = options.truncate - (2 + flags.length), source = value.source;
  return options.stylize(`/${truncate22(source, sourceLength)}/${flags}`, "regexp");
}
...
function inspectSet22(set2, options) {
  return set2.size === 0 ? "Set{}" : (options.truncate -= 7, `Set{ ${inspectList22(arrayFromSet22(set2), options)} }`);
}
...
function inspectString22(string, options) {
  return stringEscapeChars22.test(string) && (string = string.replace(stringEscapeChars22, escape3)), options.stylize(`'${truncate22(string, options.truncate - 2)}'`, "string");
}
function inspectSymbol22(value) {
  return "description" in Symbol.prototype ? value.description ? `Symbol(${value.description})` : "Symbol()" : value.toString();
}
var getPromiseValue22 = () => "Promise{…}";
try {
  let { getPromiseDetails, kPending, kRejected } = process.binding("util");
  Array.isArray(getPromiseDetails(Promise.resolve())) && (getPromiseValue22 = (value, options) => {
    let [state, innerValue] = getPromiseDetails(value);
    return state === kPending ? "Promise{<pending>}" : `Promise${state === kRejected ? "!" : ""}{${options.inspect(innerValue, options)}}`;
  });
} catch {
...
function inspectObject32(object, options) {
  let properties = Object.getOwnPropertyNames(object), symbols = Object.getOwnPropertySymbols ? Object.getOwnPropertySymbols(object) : [];
  if (properties.length === 0 && symbols.length === 0) return "{}";
  if (options.truncate -= 4, options.seen = options.seen || [], options.seen.indexOf(object) >= 0) return "[Circular]";
  options.seen.push(object);
  let propertyContents = inspectList22(properties.map((key) => [key, object[key]]), options, inspectProperty22), symbolContents = inspectList22(symbols.map((key) => [key, object[key]]), options, inspectProperty22);
  options.seen.pop();
  let sep = "";
  return propertyContents && symbolContents && (sep = ", "), `{ ${propertyContents}${sep}${symbolContents} }`;
}
var toStringTag22 = typeof Symbol < "u" && Symbol.toStringTag ? Symbol.toStringTag : false;
function inspectClass22(value, options) {
  let name = "";
  return toStringTag22 && toStringTag22 in value && (name = value[toStringTag22]), name = name || value.constructor.name, (!name || name === "_class") && (name = "<Anonymous Class>"), options.truncate -= name.length, `${name}${inspectObject32(value, options)}`;
}
function inspectArguments22(args, options) {
  return args.length === 0 ? "Arguments[]" : (options.truncate -= 13, `Arguments[ ${inspectList22(args, options)} ]`);
}
...
function inspectObject4(error, options) {
  let properties = Object.getOwnPropertyNames(error).filter((key) => errorKeys22.indexOf(key) === -1), name = error.name;
  options.truncate -= name.length;
  let message = "";
  typeof error.message == "string" ? message = truncate22(error.message, options.truncate) : properties.unshift("message"), message = message ? `: ${message}` : "", options.truncate -= message.length + 5;
  let propertyContents = inspectList22(properties.map((key) => [key, error[key]]), options, inspectProperty22);
  return `${name}${message}${propertyContents ? ` { ${propertyContents} }` : ""}`;
}
function inspectAttribute22([key, value], options) {
  return options.truncate -= 3, value ? `${options.stylize(String(key), "yellow")}=${options.stylize(`"${value}"`, "string")}` : `${options.stylize(String(key), "yellow")}`;
}
function inspectHTMLCollection22(collection, options) {
  return inspectList22(collection, options, inspectHTML22, `
`);
}
function inspectHTML22(element, options) {
  let properties = element.getAttributeNames(), name = element.tagName.toLowerCase(), head = options.stylize(`<${name}`, "special"), headClose = options.stylize(">", "special"), tail = options.stylize(`</${name}>`, "special");
  options.truncate -= name.length * 2 + 5;
  let propertyContents = "";
  properties.length > 0 && (propertyContents += " ", propertyContents += inspectList22(properties.map((key) => [key, element.getAttribute(key)]), options, inspectAttribute22, " ")), options.truncate -= propertyContents.length;
  let truncate3 = options.truncate, children = inspectHTMLCollection22(element.children, options);
...
var stringTagMap22 = {};
var baseTypesMap22 = { undefined: (value, options) => options.stylize("undefined", "undefined"), null: (value, options) => options.stylize("null", "null"), boolean: (value, options) => options.stylize(String(value), "boolean"), Boolean: (value, options) => options.stylize(String(value), "boolean"), number: inspectNumber22, Number: inspectNumber22, bigint: inspectBigInt22, BigInt: inspectBigInt22, string: inspectString22, String: inspectString22, function: inspectFunction22, Function: inspectFunction22, symbol: inspectSymbol22, Symbol: inspectSymbol22, Array: inspectArray22, Date: inspectDate22, Map: inspectMap22, Set: inspectSet22, RegExp: inspectRegExp22, Promise: promise_default22, WeakSet: (value, options) => options.stylize("WeakSet{…}", "special"), WeakMap: (value, options) => options.stylize("WeakMap{…}", "special"), Arguments: inspectArguments22, Int8Array: inspectTypedArray22, Uint8Array: inspectTypedArray22, Uint8ClampedArray: inspectTypedArray22, Int16Array: inspectTypedArray22, Uint16Array: inspectTypedArray22, Int32Array: inspectTypedArray22, Uint32Array: inspectTypedArray22, Float32Array: inspectTypedArray22, Float64Array: inspectTypedArray22, Generator: () => "", DataView: () => "", ArrayBuffer: () => "", Error: inspectObject4, HTMLCollection: inspectHTMLCollection22, NodeList: inspectHTMLCollection22 };
var inspectCustom22 = (value, options, type5) => chaiInspect22 in value && typeof value[chaiInspect22] == "function" ? value[chaiInspect22](options) : nodeInspect22 && nodeInspect22 in value && typeof value[nodeInspect22] == "function" ? value[nodeInspect22](options.depth, options) : "inspect" in value && typeof value.inspect == "function" ? value.inspect(options.depth, options) : "constructor" in value && constructorMap22.has(value.constructor) ? constructorMap22.get(value.constructor)(value, options) : stringTagMap22[type5] ? stringTagMap22[type5](value, options) : "";
var toString22 = Object.prototype.toString;
function inspect32(value, opts = {}) {
  let options = normaliseOptions22(opts, inspect32), { customInspect } = options, type5 = value === null ? "null" : typeof value;
  if (type5 === "object" && (type5 = toString22.call(value).slice(8, -1)), type5 in baseTypesMap22) return baseTypesMap22[type5](value, options);
  if (customInspect && value) {
    let output = inspectCustom22(value, options, type5);
    if (output) return typeof output == "string" ? output : inspect32(output, options);
  }
  let proto = value ? Object.getPrototypeOf(value) : false;
  return proto === Object.prototype || proto === null ? inspectObject32(value, options) : value && typeof HTMLElement == "function" && value instanceof HTMLElement ? inspectHTML22(value, options) : "constructor" in value ? value.constructor !== Object ? inspectClass22(value, options) : inspectObject32(value, options) : value === Object(value) ? inspectObject32(value, options) : options.stylize(String(value), type5);
}
...
function p2(n = false) {
  ...
  for (let [r, t] of h2) u2[r] = e ? g3(d2(t[0]), d2(t[1]), t[2]) : a2;
  return u2;
}
var f22 = p2(false);
...
function printIteratorEntries2(iterator, config3, indentation, depth, refs, printer22, separator = ": ") {
  let result = "", width = 0, current = iterator.next();
  if (!current.done) {
    result += config3.spacingOuter;
    let indentationNext = indentation + config3.indent;
...
function printIteratorValues2(iterator, config3, indentation, depth, refs, printer22) {
  let result = "", width = 0, current = iterator.next();
  if (!current.done) {
    result += config3.spacingOuter;
    let indentationNext = indentation + config3.indent;
...
function printListItems2(list, config3, indentation, depth, refs, printer22) {
  let result = "";
  list = list instanceof ArrayBuffer ? new DataView(list) : list;
  let isDataView = (l) => l instanceof DataView, length = isDataView(list) ? list.byteLength : list.length;
  if (length > 0) {
...
function printObjectProperties2(val, config3, indentation, depth, refs, printer22) {
...
var SPACE$22 = " ";
var serialize$52 = (val, config3, indentation, depth, refs, printer22) => {
  let stringedValue = val.toString();
  if (stringedValue === "ArrayContaining" || stringedValue === "ArrayNotContaining") return ++depth > config3.maxDepth ? `[${stringedValue}]` : `${stringedValue + SPACE$22}[${printListItems2(val.sample, config3, indentation, depth, refs, printer22)}]`;
  if (stringedValue === "ObjectContaining" || stringedValue === "ObjectNotContaining") return ++depth > config3.maxDepth ? `[${stringedValue}]` : `${stringedValue + SPACE$22}{${printObjectProperties2(val.sample, config3, indentation, depth, refs, printer22)}}`;
  if (stringedValue === "StringMatching" || stringedValue === "StringNotMatching" || stringedValue === "StringContaining" || stringedValue === "StringNotContaining") return stringedValue + SPACE$22 + printer22(val.sample, config3, indentation, depth, refs);
  if (typeof val.toAsymmetricMatcher != "function") throw new TypeError(`Asymmetric matcher ${val.constructor.name} does not implement toAsymmetricMatcher()`);
  return val.toAsymmetricMatcher();
};
...
function testName2(name) {
  return OBJECT_NAMES2.has(name) || ARRAY_REGEXP2.test(name);
}
...
var serialize$42 = (collection, config3, indentation, depth, refs, printer22) => {
  let name = collection.constructor.name;
  return ++depth > config3.maxDepth ? `[${name}]` : (config3.min ? "" : name + SPACE$12) + (OBJECT_NAMES2.has(name) ? `{${printObjectProperties2(isNamedNodeMap2(collection) ? [...collection].reduce((props, attribute) => (props[attribute.name] = attribute.value, props), {}) : { ...collection }, config3, indentation, depth, refs, printer22)}}` : `[${printListItems2([...collection], config3, indentation, depth, refs, printer22)}]`);
};
var plugin$42 = { serialize: serialize$42, test: test$42 };
function escapeHTML2(str) {
  return str.replaceAll("<", "&lt;").replaceAll(">", "&gt;");
}
function printProps2(keys2, props, config3, indentation, depth, refs, printer22) {
  let indentationNext = indentation + config3.indent, colors = config3.colors;
  return keys2.map((key) => {
    let value = props[key], printed = printer22(value, config3, indentationNext, depth, refs);
    return typeof value != "string" && (printed.includes(`
...
function printChildren2(children, config3, indentation, depth, refs, printer22) {
  return children.map((child) => config3.spacingOuter + indentation + (typeof child == "string" ? printText2(child, config3) : printer22(child, config3, indentation, depth, refs))).join("");
}
...
var serialize$32 = (node, config3, indentation, depth, refs, printer22) => {
  ...
  return ++depth > config3.maxDepth ? printElementAsLeaf2(type5, config3) : printElement2(type5, printProps2(nodeIsFragment2(node) ? [] : Array.from(node.attributes, (attr) => attr.name).sort(), nodeIsFragment2(node) ? {} : [...node.attributes].reduce((props, attribute) => (props[attribute.name] = attribute.value, props), {}), config3, indentation + config3.indent, depth, refs, printer22), printChildren2(Array.prototype.slice.call(node.childNodes || node.children), config3, indentation + config3.indent, depth, refs, printer22), config3, indentation);
};
...
var SPACE2 = " ";
var LAZY2 = "…";
function printImmutableEntries2(val, config3, indentation, depth, refs, printer22, type5) {
  return ++depth > config3.maxDepth ? printAsLeaf2(getImmutableName2(type5)) : `${getImmutableName2(type5) + SPACE2}{${printIteratorEntries2(val.entries(), config3, indentation, depth, refs, printer22)}}`;
}
...
function printImmutableRecord2(val, config3, indentation, depth, refs, printer22) {
  let name = getImmutableName2(val._name || "Record");
  return ++depth > config3.maxDepth ? printAsLeaf2(name) : `${name + SPACE2}{${printIteratorEntries2(getRecordEntries2(val), config3, indentation, depth, refs, printer22)}}`;
}
function printImmutableSeq2(val, config3, indentation, depth, refs, printer22) {
  let name = getImmutableName2("Seq");
  return ++depth > config3.maxDepth ? printAsLeaf2(name) : val[IS_KEYED_SENTINEL2] ? `${name + SPACE2}{${val._iter || val._object ? printIteratorEntries2(val.entries(), config3, indentation, depth, refs, printer22) : LAZY2}}` : `${name + SPACE2}[${val._iter || val._array || val._collection || val._iterable ? printIteratorValues2(val.values(), config3, indentation, depth, refs, printer22) : LAZY2}]`;
}
function printImmutableValues2(val, config3, indentation, depth, refs, printer22, type5) {
  return ++depth > config3.maxDepth ? printAsLeaf2(getImmutableName2(type5)) : `${getImmutableName2(type5) + SPACE2}[${printIteratorValues2(val.values(), config3, indentation, depth, refs, printer22)}]`;
}
var serialize$22 = (val, config3, indentation, depth, refs, printer22) => val[IS_MAP_SENTINEL2] ? printImmutableEntries2(val, config3, indentation, depth, refs, printer22, val[IS_ORDERED_SENTINEL2] ? "OrderedMap" : "Map") : val[IS_LIST_SENTINEL2] ? printImmutableValues2(val, config3, indentation, depth, refs, printer22, "List") : val[IS_SET_SENTINEL2] ? printImmutableValues2(val, config3, indentation, depth, refs, printer22, val[IS_ORDERED_SENTINEL2] ? "OrderedSet" : "Set") : val[IS_STACK_SENTINEL2] ? printImmutableValues2(val, config3, indentation, depth, refs, printer22, "Stack") : val[IS_SEQ_SENTINEL2] ? printImmutableSeq2(val, config3, indentation, depth, refs, printer22) : printImmutableRecord2(val, config3, indentation, depth, refs, printer22);
...
var serialize$12 = (element, config3, indentation, depth, refs, printer22) => ++depth > config3.maxDepth ? printElementAsLeaf2(getType22(element), config3) : printElement2(getType22(element), printProps2(getPropKeys$12(element), element.props, config3, indentation + config3.indent, depth, refs, printer22), printChildren2(getChildren2(element.props.children), config3, indentation + config3.indent, depth, refs, printer22), config3, indentation);
...
var serialize2 = (object, config3, indentation, depth, refs, printer22) => ++depth > config3.maxDepth ? printElementAsLeaf2(object.type, config3) : printElement2(object.type, object.props ? printProps2(getPropKeys2(object), object.props, config3, indentation + config3.indent, depth, refs, printer22) : "", object.children ? printChildren2(object.children, config3, indentation + config3.indent, depth, refs, printer22) : "", config3, indentation);
...
function printComplexValue2(val, config3, indentation, depth, refs, hasCalledToJSON) {
  ...
  let toStringed = toString32.call(val);
  return toStringed === "[object Arguments]" ? hitMaxDepth ? "[Arguments]" : `${min ? "" : "Arguments "}[${printListItems2(val, config3, indentation, depth, refs, printer2)}]` : isToStringedArrayType2(toStringed) ? hitMaxDepth ? `[${val.constructor.name}]` : `${min || !config3.printBasicPrototype && val.constructor.name === "Array" ? "" : `${val.constructor.name} `}[${printListItems2(val, config3, indentation, depth, refs, printer2)}]` : toStringed === "[object Map]" ? hitMaxDepth ? "[Map]" : `Map {${printIteratorEntries2(val.entries(), config3, indentation, depth, refs, printer2, " => ")}}` : toStringed === "[object Set]" ? hitMaxDepth ? "[Set]" : `Set {${printIteratorValues2(val.values(), config3, indentation, depth, refs, printer2)}}` : hitMaxDepth || isWindow2(val) ? `[${getConstructorName22(val)}]` : `${min || !config3.printBasicPrototype && getConstructorName22(val) === "Object" ? "" : `${getConstructorName22(val)} `}{${printObjectProperties2(val, config3, indentation, depth, refs, printer2)}}`;
}
function isNewPlugin2(plugin22) {
  return plugin22.serialize != null;
}
function printPlugin2(plugin22, val, config3, indentation, depth, refs) {
  let printed;
  try {
    printed = isNewPlugin2(plugin22) ? plugin22.serialize(val, config3, indentation, depth, refs, printer2) : plugin22.print(val, (valChild) => printer2(valChild, config3, indentation, depth, refs), (str) => {
      let indentationNext = indentation + config3.indent;
...
function printer2(val, config3, indentation, depth, refs, hasCalledToJSON) {
  ...
  if (plugin22 !== null) return printPlugin2(plugin22, val, config3, indentation, depth, refs);
...
function validateOptions2(options) {
  for (let key of Object.keys(options)) if (!Object.prototype.hasOwnProperty.call(DEFAULT_OPTIONS2, key)) throw new Error(`pretty-format: Unknown option "${key}".`);
  if (options.min && options.indent !== void 0 && options.indent !== 0) throw new Error('pretty-format: Options "min" and "indent" cannot be used together.');
}
function getColorsHighlight2() {
  return DEFAULT_THEME_KEYS2.reduce((colors, key) => {
    let value = DEFAULT_THEME2[key], color = value && f22[value];
    if (color && typeof color.close == "string" && typeof color.open == "string") colors[key] = color;
    else throw new Error(`pretty-format: Option "theme" has a key "${key}" whose value "${value}" is undefined in ansi-styles.`);
    return colors;
  }, /* @__PURE__ */ Object.create(null));
...
function getColorsEmpty2() {
  return DEFAULT_THEME_KEYS2.reduce((colors, key) => (colors[key] = { close: "", open: "" }, colors), /* @__PURE__ */ Object.create(null));
}
function getPrintFunctionName2(options) {
  return (options == null ? void 0 : options.printFunctionName) ?? DEFAULT_OPTIONS2.printFunctionName;
}
function getEscapeRegex2(options) {
  return (options == null ? void 0 : options.escapeRegex) ?? DEFAULT_OPTIONS2.escapeRegex;
}
function getEscapeString2(options) {
  return (options == null ? void 0 : options.escapeString) ?? DEFAULT_OPTIONS2.escapeString;
}
function getConfig2(options) {
  return { callToJSON: (options == null ? void 0 : options.callToJSON) ?? DEFAULT_OPTIONS2.callToJSON, colors: (options == null ? void 0 : options.highlight) ? getColorsHighlight2() : getColorsEmpty2(), compareKeys: typeof (options == null ? void 0 : options.compareKeys) == "function" || (options == null ? void 0 : options.compareKeys) === null ? options.compareKeys : DEFAULT_OPTIONS2.compareKeys, escapeRegex: getEscapeRegex2(options), escapeString: getEscapeString2(options), indent: (options == null ? void 0 : options.min) ? "" : createIndent2((options == null ? void 0 : options.indent) ?? DEFAULT_OPTIONS2.indent), maxDepth: (options == null ? void 0 : options.maxDepth) ?? DEFAULT_OPTIONS2.maxDepth, maxWidth: (options == null ? void 0 : options.maxWidth) ?? DEFAULT_OPTIONS2.maxWidth, min: (options == null ? void 0 : options.min) ?? DEFAULT_OPTIONS2.min, plugins: (options == null ? void 0 : options.plugins) ?? DEFAULT_OPTIONS2.plugins, printBasicPrototype: (options == null ? void 0 : options.printBasicPrototype) ?? true, printFunctionName: getPrintFunctionName2(options), spacingInner: (options == null ? void 0 : options.min) ? " " : `
`, spacingOuter: (options == null ? void 0 : options.min) ? "" : `
` };
}
...
function format3(val, options) {
  if (options && (validateOptions2(options), options.plugins)) {
    let plugin22 = findPlugin2(options.plugins, val);
    if (plugin22 !== null) return printPlugin2(plugin22, val, getConfig2(options), "", 0, []);
  }
  let basicResult = printBasicValue2(val, getPrintFunctionName2(options), getEscapeRegex2(options), getEscapeString2(options));
  return basicResult !== null ? basicResult : printComplexValue2(val, getConfig2(options), "", 0, []);
}
...
function stringify2(object, maxDepth = 10, { maxLength, ...options } = {}) {
  let MAX_LENGTH = maxLength ?? 1e4, result;
  try {
    result = format3(object, { maxDepth, escapeString: false, plugins: PLUGINS3, ...options });
  } catch {
    result = format3(object, { callToJSON: false, maxDepth, escapeString: false, plugins: PLUGINS3, ...options });
  }
  return result.length >= MAX_LENGTH && maxDepth > 1 ? stringify2(object, Math.floor(maxDepth / 2)) : result;
}
...
function format22(...args) {
  ...
  let len = args.length, i = 1, template = args[0], str = String(template).replace(formatRegExp2, (x2) => {
    if (x2 === "%%") return "%";
    if (i >= len) return x2;
    switch (x2) {
      case "%s": {
...
function inspect4(obj, options = {}) {
  return options.truncate === 0 && (options.truncate = Number.POSITIVE_INFINITY), inspect32(obj, options);
}
...
function joinAlignedDiffsNoExpand2(diffs, options) {
  let iLength = diffs.length, nContextLines = options.contextLines, nContextLines2 = nContextLines + nContextLines, jLength = iLength, hasExcessAtStartOrEnd = false, nExcessesBetweenChanges = 0, i = 0;
  for (; i !== iLength; ) {
    let iStart = i;
    for (; i !== iLength && diffs[i][0] === DIFF_EQUAL2; ) i += 1;
    if (iStart !== i) if (iStart === 0) i > nContextLines && (jLength -= i - nContextLines, hasExcessAtStartOrEnd = true);
    else if (i === iLength) {
      let n = i - iStart;
      n > nContextLines && (jLength -= n - nContextLines, hasExcessAtStartOrEnd = true);
  ...
  let aStart = 0, bStart = 0, aEnd = 0, bEnd = 0, pushCommonLine = (line) => {
    let j = lines.length;
    lines.push(printCommonLine2(line, j === 0 || j === jLast, options)), aEnd += 1, bEnd += 1;
  }, pushDeleteLine = (line) => {
    let j = lines.length;
    lines.push(printDeleteLine2(line, j === 0 || j === jLast, options)), aEnd += 1;
  }, pushInsertLine = (line) => {
    let j = lines.length;
    lines.push(printInsertLine2(line, j === 0 || j === jLast, options)), bEnd += 1;
  };
  for (i = 0; i !== iLength; ) {
    let iStart = i;
    for (; i !== iLength && diffs[i][0] === DIFF_EQUAL2; ) i += 1;
    if (iStart !== i) if (iStart === 0) {
      ...
      if (nCommon > nContextLines2) {
        let iEnd = iStart + nContextLines;
        for (let iCommon = iStart; iCommon !== iEnd; iCommon += 1) pushCommonLine(diffs[iCommon][1]);
        lines[jPatchMark] = createPatchMark2(aStart, aEnd, bStart, bEnd, options), jPatchMark = lines.length, lines.push("");
        let nOmit = nCommon - nContextLines2;
        aStart = aEnd + nOmit, bStart = bEnd + nOmit, aEnd = aStart, bEnd = bStart;
        for (let iCommon = i - nContextLines; iCommon !== i; iCommon += 1) pushCommonLine(diffs[iCommon][1]);
      } else for (let iCommon = iStart; iCommon !== i; iCommon += 1) pushCommonLine(diffs[iCommon][1]);
    }
    for (; i !== iLength && diffs[i][0] === DIFF_DELETE2; ) pushDeleteLine(diffs[i][1]), i += 1;
    for (; i !== iLength && diffs[i][0] === DIFF_INSERT2; ) pushInsertLine(diffs[i][1]), i += 1;
  }
  return hasPatch && (lines[jPatchMark] = createPatchMark2(aStart, aEnd, bStart, bEnd, options)), lines.join(`
`);
}
function joinAlignedDiffsExpand2(diffs, options) {
  return diffs.map((diff22, i, diffs2) => {
    let line = diff22[1], isFirstOrLast = i === 0 || i === diffs2.length - 1;
    switch (diff22[0]) {
      case DIFF_DELETE2:
        return printDeleteLine2(line, isFirstOrLast, options);
      case DIFF_INSERT2:
        return printInsertLine2(line, isFirstOrLast, options);
      default:
        return printCommonLine2(line, isFirstOrLast, options);
    }
  }).join(`
`);
}
...
function normalizeDiffOptions2(options = {}) {
  return { ...getDefaultOptions2(), ...options, compareKeys: getCompareKeys2(options.compareKeys), contextLines: getContextLines2(options.contextLines) };
}
...
function printDiffLines2(diffs, truncated, options) {
  return printAnnotation2(options, countChanges2(diffs)) + (options.expand ? joinAlignedDiffsExpand2(diffs, options) : joinAlignedDiffsNoExpand2(diffs, options)) + (truncated ? options.truncateAnnotationColor(`
${options.truncateAnnotation}`) : "");
}
function diffLinesUnified3(aLines, bLines, options) {
  let normalizedOptions = normalizeDiffOptions2(options), [diffs, truncated] = diffLinesRaw2(isEmptyString2(aLines) ? [] : aLines, isEmptyString2(bLines) ? [] : bLines, normalizedOptions);
  return printDiffLines2(diffs, truncated, normalizedOptions);
}
function diffLinesUnified22(aLinesDisplay, bLinesDisplay, aLinesCompare, bLinesCompare, options) {
  if (isEmptyString2(aLinesDisplay) && isEmptyString2(aLinesCompare) && (aLinesDisplay = [], aLinesCompare = []), isEmptyString2(bLinesDisplay) && isEmptyString2(bLinesCompare) && (bLinesDisplay = [], bLinesCompare = []), aLinesDisplay.length !== aLinesCompare.length || bLinesDisplay.length !== bLinesCompare.length) return diffLinesUnified3(aLinesDisplay, bLinesDisplay, options);
  let [diffs, truncated] = diffLinesRaw2(aLinesCompare, bLinesCompare, options), aIndex = 0, bIndex = 0;
  return diffs.forEach((diff22) => {
    switch (diff22[0]) {
      case DIFF_DELETE2:
        diff22[1] = aLinesDisplay[aIndex], aIndex += 1;
  ...
  }), printDiffLines2(diffs, truncated, normalizeDiffOptions2(options));
}
function diffLinesRaw2(aLines, bLines, options) {
...
function diffStrings2(a22, b, options) {
  let truncate3 = (options == null ? void 0 : options.truncateThreshold) ?? false, truncateThreshold = Math.max(Math.floor((options == null ? void 0 : options.truncateThreshold) ?? 0), 0), aLength = a22.length, bLength = b.length;
  if (truncate3) {
    let aMultipleLines = a22.includes(`
`), bMultipleLines = b.includes(`
`), aNewLineSymbol = getNewLineSymbol2(a22), bNewLineSymbol = getNewLineSymbol2(b), _a2 = aMultipleLines ? `${a22.split(aNewLineSymbol, truncateThreshold).join(aNewLineSymbol)}
` : a22, _b = bMultipleLines ? `${b.split(bNewLineSymbol, truncateThreshold).join(bNewLineSymbol)}
` : b;
    aLength = _a2.length, bLength = _b.length;
  }
  ...
  return _default(aLength, bLength, isCommon, (nCommon, aCommon, bCommon) => {
    aIndex !== aCommon && diffs.push(new Diff2(DIFF_DELETE2, a22.slice(aIndex, aCommon))), bIndex !== bCommon && diffs.push(new Diff2(DIFF_INSERT2, b.slice(bIndex, bCommon))), aIndex = aCommon + nCommon, bIndex = bCommon + nCommon, diffs.push(new Diff2(DIFF_EQUAL2, b.slice(bCommon, bIndex)));
  }), aIndex !== aLength && diffs.push(new Diff2(DIFF_DELETE2, a22.slice(aIndex))), bIndex !== bLength && diffs.push(new Diff2(DIFF_INSERT2, b.slice(bIndex))), [diffs, truncated];
}
...
function hasCommonDiff2(diffs, isMultiline) {
  if (isMultiline) {
    let iLast = diffs.length - 1;
    return diffs.some((diff22, i) => diff22[0] === DIFF_EQUAL2 && (i !== iLast || diff22[1] !== `
`));
  }
  return diffs.some((diff22) => diff22[0] === DIFF_EQUAL2);
}
function diffStringsUnified2(a22, b, options) {
  if (a22 !== b && a22.length !== 0 && b.length !== 0) {
    let isMultiline = a22.includes(`
`) || b.includes(`
...
` : b, true, options);
    if (hasCommonDiff2(diffs, isMultiline)) {
      let optionsNormalized = normalizeDiffOptions2(options), lines = getAlignedDiffs2(diffs, optionsNormalized.changeColor);
...
function diffStringsRaw2(a22, b, cleanup, options) {
  let [diffs, truncated] = diffStrings2(a22, b, options);
  return cleanup && diff_cleanupSemantic2(diffs), [diffs, truncated];
}
function getCommonMessage2(message, options) {
  let { commonColor } = normalizeDiffOptions2(options);
  return commonColor(message);
}
...
function diff2(a22, b, options) {
  ...
  if (expectedType !== getType32(b)) {
    let { aAnnotation, aColor, aIndicator, bAnnotation, bColor, bIndicator } = normalizeDiffOptions2(options), formatOptions = getFormatOptions2(FALLBACK_FORMAT_OPTIONS2, options), aDisplay = format3(a22, formatOptions), bDisplay = format3(b, formatOptions), aDiff = `${aColor(`${aIndicator} ${aAnnotation}:`)} 
${aDisplay}`, bDiff = `${bColor(`${bIndicator} ${bAnnotation}:`)} 
${bDisplay}`;
    return `${aDiff}
...
`), options);
    case "boolean":
    case "number":
      return comparePrimitive2(a22, b, options);
    case "map":
      return compareObjects2(sortMap2(a22), sortMap2(b), options);
    case "set":
      return compareObjects2(sortSet2(a22), sortSet2(b), options);
    default:
      return compareObjects2(a22, b, options);
  }
}
function comparePrimitive2(a22, b, options) {
  let aFormat = format3(a22, FORMAT_OPTIONS2), bFormat = format3(b, FORMAT_OPTIONS2);
...
function compareObjects2(a22, b, options) {
  let difference, hasThrown = false;
  try {
    let formatOptions = getFormatOptions2(FORMAT_OPTIONS2, options);
    difference = getObjectsDifference2(a22, b, formatOptions, options);
  } catch {
    hasThrown = true;
  }
  let noDiffMessage = getCommonMessage2(NO_DIFF_MESSAGE2, options);
  if (difference === void 0 || difference === noDiffMessage) {
    let formatOptions = getFormatOptions2(FALLBACK_FORMAT_OPTIONS2, options);
    difference = getObjectsDifference2(a22, b, formatOptions, options), difference !== noDiffMessage && !hasThrown && (difference = `${getCommonMessage2(SIMILAR_MESSAGE2, options)}

${difference}`);
  }
  return difference;
}
function getFormatOptions2(formatOptions, options) {
  let { compareKeys } = normalizeDiffOptions2(options);
  return { ...formatOptions, compareKeys };
}
function getObjectsDifference2(a22, b, formatOptions, options) {
  let formatOptionsZeroIndent = { ...formatOptions, indent: 0 }, aCompare = format3(a22, formatOptionsZeroIndent), bCompare = format3(b, formatOptionsZeroIndent);
  if (aCompare === bCompare) return getCommonMessage2(NO_DIFF_MESSAGE2, options);
  {
    let aDisplay = format3(a22, formatOptions), bDisplay = format3(b, formatOptions);
...
function printDiffOrStringify2(expected, received, options) {
  let { aAnnotation, bAnnotation } = normalizeDiffOptions2(options);
  if (typeof expected == "string" && typeof received == "string" && expected.length > 0 && received.length > 0 && expected.length <= MAX_DIFF_STRING_LENGTH2 && received.length <= MAX_DIFF_STRING_LENGTH2 && expected !== received) {
    if (expected.includes(`
`) || received.includes(`
`)) return diffStringsUnified2(received, expected, options);
...
${receivedLine}`;
  ...
  return diff2(replacedExpected, replacedActual, options);
}
function replaceAsymmetricMatcher2(actual, expected, actualReplaced = /* @__PURE__ */ new WeakSet(), expectedReplaced = /* @__PURE__ */ new WeakSet()) {
  return isReplaceable2(actual, expected) ? actualReplaced.has(actual) || expectedReplaced.has(expected) ? { replacedActual: actual, replacedExpected: expected } : (actualReplaced.add(actual), expectedReplaced.add(expected), getOwnProperties2(expected).forEach((key) => {
    let expectedValue = expected[key], actualValue = actual[key];
    if (isAsymmetricMatcher2(expectedValue)) expectedValue.asymmetricMatch(actualValue) && (actual[key] = expectedValue);
    else if (isAsymmetricMatcher2(actualValue)) actualValue.asymmetricMatch(expectedValue) && (expected[key] = actualValue);
...
function printReceived2(object) {
  return f22.red(replaceTrailingSpaces2(stringify2(object)));
}
function printExpected2(value) {
  return f22.green(replaceTrailingSpaces2(stringify2(value)));
}
function getCommonAndChangedSubstrings2(diffs, op, hasCommonDiff22) {
  return diffs.reduce((reduced, diff22) => reduced + (diff22[0] === DIFF_EQUAL2 ? diff22[1] : diff22[0] === op ? hasCommonDiff22 ? f22.inverse(diff22[1]) : diff22[1] : ""), "");
}
function d(e, t) {
...
var K = (e) => (f32(e, u, { value: { reset: () => P(e[u]) } }), e[u]);
var I = (e) => e[u] || K(e);
function g(e) {
  ...
  let t = function(...s) {
    ...
    let i, c = "ok", a22 = r.results.length;
    if (r.impl) try {
    ...
    let S = [c, i];
    return w(i) && i.then((o) => r.resolves[a22] = ["ok", o], (o) => r.resolves[a22] = ["error", o]), r.results.push(S), i;
  };
  p22(t, "_isMockFunction", true), p22(t, "length", e ? e.length : 0), p22(t, "name", e && e.name || "spy");
  let n = I(t);
...
function C22(e, t, n) {
  ...
  })(), R = k(e, s), i = Object.getPrototypeOf(e), c = i && k(i, s), a22 = R || c;
  d(a22 || s in e, `${String(s)} does not exist`);
  let S = false;
  r === "value" && a22 && !a22.value && a22.get && (r = "get", S = true, n = a22.get());
  let o;
  a22 ? o = a22[r] : r !== "value" ? o = () => e[s] : o = e[s], n || (n = o);
  let l = g(n);
  r === "value" && O(l, o);
  let h22 = (A) => {
    let { value: M, ...v } = a22 || { configurable: true, writable: true };
    r !== "value" && delete v.writable, v[r] = A, f32(e, s, v);
  }, b = () => a22 ? f32(e, s, a22) : h22(o), m2 = l[u];
  return p22(m2, "restore", b), p22(m2, "getOriginal", () => S ? o() : o), p22(m2, "willCall", (A) => (m2.impl = A, l)), h22(S ? () => (O(l, n), l) : l), x.add(l), l;
}
...
function isImmutable2(v) {
  return v && (v[IS_COLLECTION_SYMBOL2] || v[IS_RECORD_SYMBOL2]);
}
...
function serializeValue2(val, seen = /* @__PURE__ */ new WeakMap()) {
  ...
  if (Array.isArray(val)) {
    ...
    return seen.set(val, clone22), val.forEach((e, i) => {
      try {
        clone22[i] = serializeValue2(e, seen);
      } catch (err) {
        clone22[i] = getUnserializableMessage2(err);
      }
    }), clone22;
  } else {
    let clone22 = /* @__PURE__ */ Object.create(null);
    seen.set(val, clone22);
    let obj = val;
    for (; obj && obj !== OBJECT_PROTO2; ) Object.getOwnPropertyNames(obj).forEach((key) => {
      if (!(key in clone22)) try {
        clone22[key] = serializeValue2(val[key], seen);
      } catch (err) {
        delete clone22[key], clone22[key] = getUnserializableMessage2(err);
      }
    }), obj = Object.getPrototypeOf(obj);
    return clone22;
  }
...
function processError2(_err, diffOptions, seen = /* @__PURE__ */ new WeakSet()) {
  ...
  try {
    ...
    return serializeValue2(new Error(`Failed to fully serialize error: ${e == null ? void 0 : e.message}
Inner error message: ${err == null ? void 0 : err.message}`));
  }
}
...
var EXPECTED_COLOR = f22.green;
var RECEIVED_COLOR = f22.red;
var INVERTED_COLOR = f22.inverse;
var BOLD_WEIGHT = f22.bold;
var DIM_COLOR = f22.dim;
...
function printExpected22(value) {
  return EXPECTED_COLOR(replaceTrailingSpaces22(stringify2(value)));
}
...
function asymmetricMatch(a22, b) {
  ...
  if (!(asymmetricA && asymmetricB)) {
    if (asymmetricA) return a22.asymmetricMatch(b);
    if (asymmetricB) return b.asymmetricMatch(a22);
  }
}
function eq(a22, b, aStack, bStack, customTesters, hasKey2) {
  ...
  if (a22 instanceof Error && b instanceof Error) return a22.message === b.message;
  if (typeof URL == "function" && a22 instanceof URL && b instanceof URL) return a22.href === b.href;
  if (Object.is(a22, b)) return true;
  if (a22 === null || b === null) return a22 === b;
  ...
  if (typeof a22 != "object" || typeof b != "object") return false;
  if (isDomNode(a22) && isDomNode(b)) return a22.isEqualNode(b);
  let length = aStack.length;
  for (; length--; ) {
    if (aStack[length] === a22) return bStack[length] === b;
    if (bStack[length] === b) return false;
  }
  if (aStack.push(a22), bStack.push(b), className === "[object Array]" && a22.length !== b.length) return false;
  let aKeys = keys(a22, hasKey2), key, size = aKeys.length;
  if (keys(b, hasKey2).length !== size) return false;
  for (; size--; ) if (key = aKeys[size], result = hasKey2(b, key) && eq(a22[key], b[key], aStack, bStack, customTesters, hasKey2), !result) return false;
...
function isImmutableList(source) {
  return !!(source && isObjectLiteral(source) && source[IS_LIST_SENTINEL22]);
}
function isImmutableOrderedKeyed(source) {
  return !!(source && isObjectLiteral(source) && source[IS_KEYED_SENTINEL22] && source[IS_ORDERED_SENTINEL22]);
}
function isImmutableOrderedSet(source) {
  return !!(source && isObjectLiteral(source) && source[IS_SET_SENTINEL22] && source[IS_ORDERED_SENTINEL22]);
}
function isImmutableRecord(source) {
  return !!(source && isObjectLiteral(source) && source[IS_RECORD_SYMBOL22]);
}
...
function iterableEquality(a22, b, customTesters = [], aStack = [], bStack = []) {
  if (typeof a22 != "object" || typeof b != "object" || Array.isArray(a22) || Array.isArray(b) || !hasIterator(a22) || !hasIterator(b)) return;
  if (a22.constructor !== b.constructor) return false;
  let length = aStack.length;
  for (; length--; ) if (aStack[length] === a22) return bStack[length] === b;
  ...
  function iterableEqualityWithStack(a222, b2) {
    return iterableEquality(a222, b2, [...customTesters], [...aStack], [...bStack]);
  }
  if (a22.size !== void 0) {
    if (a22.size !== b.size) return false;
    if (isA("Set", a22) || isImmutableUnorderedSet(a22)) {
      let allFound = true;
      for (let aValue of a22) if (!b.has(aValue)) {
        let has = false;
      ...
      let allFound = true;
      for (let aEntry of a22) if (!b.has(aEntry[0]) || !equals(aEntry[1], b.get(aEntry[0]), filteredCustomTesters)) {
        let has = false;
        for (let bEntry of b) {
...
function typeEquality(a22, b) {
  if (!(a22 == null || b == null || a22.constructor === b.constructor)) return false;
}
function arrayBufferEquality(a22, b) {
  let dataViewA = a22, dataViewB = b;
  if (!(a22 instanceof DataView && b instanceof DataView)) {
    if (!(a22 instanceof ArrayBuffer) || !(b instanceof ArrayBuffer)) return;
    try {
...
function sparseArrayEquality(a22, b, customTesters = []) {
  if (!Array.isArray(a22) || !Array.isArray(b)) return;
  ...
  let stripped = 0, getObjectSubsetWithContext = (seenReferences = /* @__PURE__ */ new WeakMap()) => (object2, subset2) => {
    if (Array.isArray(object2)) {
      ...
      if (isObject2(object2) && isObject2(subset2)) {
        ...
        for (let key of getObjectKeys(object2)) hasPropertyInObject(subset2, key) ? trimmed[key] = seenReferences.has(object2[key]) ? seenReferences.get(object2[key]) : getObjectSubsetWithContext(seenReferences)(object2[key], subset2[key]) : seenReferences.has(object2[key]) || (stripped += 1, isObject2(object2[key]) && (stripped += getObjectKeys(object2[key]).length), getObjectSubsetWithContext(seenReferences)(object2[key], subset2[key]));
...
(${stripped} matching ${stripped === 1 ? "property" : "properties"} omitted from actual)`;
  ...
  }, formatCalls = (spy, msg, showActualCall) => (spy.mock.calls && (msg += f22.gray(`

Received: 

${spy.mock.calls.map((callArg, i) => {
    let methodCall = f22.bold(`  ${ordinalOf(i + 1)} ${spy.getMockName()} call:
...
Number of calls: ${f22.bold(spy.mock.calls.length)}
`), msg), formatReturns = (spy, results, msg, showActualReturn) => (msg += f22.gray(`

Received: 

${results.map((callReturn, i) => {
    let methodCall = f22.bold(`  ${ordinalOf(i + 1)} ${spy.getMockName()} call return:
...
var prettyFormat = __toESM2(require_build());
...
function arrayFrom2(arrayLike, mapFn) {
  ...
  for (var len = toLength2(items.length), A = isCallable2(C3) ? Object(new C3(len)) : new Array(len), k2 = 0, kValue; k2 < len; ) kValue = items[k2], mapFn ? A[k2] = mapFn(kValue, k2) : A[k2] = kValue, k2 += 1;
  return A.length = len, A;
}
...
var import_aria_query3 = __toESM2(require_lib());
var import_lz_string = __toESM2(require_lz_string());
function escapeHTML22(str) {
  return str.replace(/</g, "&lt;").replace(/>/g, "&gt;");
}
var printProps22 = (keys2, props, config3, indentation, depth, refs, printer22) => {
  let indentationNext = indentation + config3.indent, colors = config3.colors;
  return keys2.map((key) => {
    let value = props[key], printed = printer22(value, config3, indentationNext, depth, refs);
    return typeof value != "string" && (printed.indexOf(`
...
var printChildren22 = (children, config3, indentation, depth, refs, printer22) => children.map((child) => {
  let printedChild = typeof child == "string" ? printText22(child, config3) : printer22(child, config3, indentation, depth, refs);
...
function createDOMElementFilter(filterNode) {
  return { test: (val) => {
  ...
  }, serialize: (node, config3, indentation, depth, refs, printer22) => {
    ...
    return ++depth > config3.maxDepth ? printElementAsLeaf22(type5, config3) : printElement22(type5, printProps22(nodeIsFragment22(node) ? [] : Array.from(node.attributes).map((attr) => attr.name).sort(), nodeIsFragment22(node) ? {} : Array.from(node.attributes).reduce((props, attribute) => (props[attribute.name] = attribute.value, props), {}), config3, indentation + config3.indent, depth, refs, printer22), printChildren22(Array.prototype.slice.call(node.childNodes || node.children).filter(filterNode), config3, indentation + config3.indent, depth, refs, printer22), config3, indentation);
  } };
}
var chalk2 = null;
...
function filterCommentsAndDefaultIgnoreTagsTags(value) {
  return value.nodeType !== COMMENT_NODE22 && (value.nodeType !== ELEMENT_NODE22 || !value.matches(getConfig22().defaultIgnore));
}
function prettyDOM(dom, maxLength, options) {
  if (options === void 0 && (options = {}), dom || (dom = getDocument().body), typeof maxLength != "number" && (maxLength = typeof process < "u" && typeof process.env < "u" && process.env.DEBUG_PRINT_LIMIT || 7e3), maxLength === 0) return "";
  ...
  let { filterNode = filterCommentsAndDefaultIgnoreTagsTags, ...prettyFormatOptions } = options, debugContent = prettyFormat.format(dom, { plugins: [createDOMElementFilter(filterNode), DOMCollection32], printFunctionName: false, highlight: shouldHighlight(), ...prettyFormatOptions });
...
function getNodeText(node) {
  return node.matches("input[type=submit], input[type=button], input[type=reset]") ? node.value : Array.from(node.childNodes).filter((child) => child.nodeType === TEXT_NODE22 && !!child.textContent).map((c) => c.textContent).join("");
}
...
function waitForWrapper(callback, options) {
  ...
  return getConfig22().asyncWrapper(() => waitFor(callback, { stackTraceError, ...options }));
}
...
function makeGetAllQuery(allQuery, getMissingError2) {
  return function(container) {
    ...
    let els = allQuery(container, ...args);
    if (!els.length) throw getConfig22().getElementError(getMissingError2(container, ...args), container);
    return els;
  };
}
...
var wrapSingleQueryWithSuggestion = (query, queryAllByName, variant) => function(container) {
  ...
  let element = query(container, ...args), [{ suggest = getConfig22().throwSuggestions } = {}] = args.slice(-1);
  if (element && suggest) {
...
var wrapAllByQueryWithSuggestion = (query, queryAllByName, variant) => function(container) {
  ...
  let els = query(container, ...args), [{ suggest = getConfig22().throwSuggestions } = {}] = args.slice(-1);
...
var queryAllByRole = function(container, role, _temp) {
  let { hidden = getConfig22().defaultHidden, name, description, queryFallbacks = false, selected, busy, checked, pressed, current, level, expanded, value: { now: valueNow, min: valueMin, max: valueMax, text: valueText } = {} } = _temp === void 0 ? {} : _temp;
...
var getMissingError$1 = function(container, role, _temp3) {
  let { hidden = getConfig22().defaultHidden, name, description } = _temp3 === void 0 ? {} : _temp3;
  if (getConfig22()._disableExpensiveErrorDiagnostics) return 'Unable to find role="' + role + '"' + getNameHint(name);
  let roles3 = "";
...
function buildTimeValue(value) {
  ...
  return value.length === 2 ? value : build22(onlyDigitsValue, 2);
}
...
function isValidDateOrTimeValue(element, value) {
  ...
  return clone22.value = value, clone22.value === value;
}
...
testingLibrary.screen = new Proxy(testingLibrary.screen, { get(target, prop, receiver) {
  return import_client_logger2.once.warn(esm_default`
...
import_global2.global.__STORYBOOK_TEST_LOADERS__ = [resetAllMocksLoader, nameSpiesAndWrapActionsInSpies, enhanceContext];
import_global2.global.__STORYBOOK_TEST_ON_MOCK_CALL__ = onMockCall;
...
//# sourceMappingURL=chunk-FN34KQTJ.js.map
